...
**Prompt:**
Build the idle-session-timeout feature (new scope, plan already reviewed and locked — proceed straight to implementation, no further brainstorming on the *what*, but keep `superpowers`' TDD/verification discipline for the *how*). Locked decisions, not to be re-litigated: 15 min inactivity → warning toast, 20 min → sign out; activity signals `mousemove`/`keydown`/`scroll`/`click`/`touchstart` with a throttled reset (don't re-arm on every raw event); authenticated routes only, mounted in `ProtectedRoute.tsx`, never on public `/auth/*`; warning uses the existing Sonner toast with a "Stay signed in" action button that immediately resets the idle clock (not a passive auto-dismissing notice); sign-out reuses `useAuth().signOut()`, no duplicated logic. The part that actually matters: Razorpay's checkout is a third-party iframe overlay, so a user typing card details inside it generates zero activity events on our own document — add a shared `isCheckoutOpen` flag set true right before `checkout.open()` and false in both `verifyPayment()`'s completion (success or failure) and the existing `modal.ondismiss`, and have the idle timer **skip its own tick entirely (suspend, not reset)** while the flag is true. Placement of the flag is my judgment call (AuthContext vs. a standalone module) but the suspend-not-reset contract is fixed. Cross-tab idle tracking is an accepted per-tab limitation — explicitly do **not** build `storage`/`BroadcastChannel` syncing, and don't touch AuthContext's `onAuthStateChange`/`applySession` logic (DEC-084) beyond what's strictly needed. Don't change the 15/20 thresholds without checking back. No IIT Capstone doc edits — those are handled separately.

**Skills invoked:**
- `superpowers` — for its verification-before-completion discipline specifically. Per CLAUDE.md's own instruction, stating explicitly rather than silently skipping: **this repo still has no test runner configured**, so the Iron Law ("no production code without a failing test first") could not be followed literally for this feature either. That remains an accepted, flagged gap for the capstone timeline, not an oversight — the same standing gap noted in earlier entries. Its systematic-debugging phase didn't apply here (new feature, no bug to root-cause).
- `impeccable` — **not** invoked, deliberately. This ships no new visual surface: the only rendered output is a Sonner toast using the existing `Toaster` (`src/components/ui/sonner.tsx`) with its already-defined `warning`/`info` icons, tokens, and styling. No new component, colors, spacing, or motion, so there was nothing for the absolute-bans check or the AI-slop test to evaluate.

**What was done:**
- **New** `src/lib/checkout-state.ts` — module-level `checkoutOpen` boolean with `setCheckoutOpen()`/`isCheckoutOpen()`. Placement reasoning recorded in the file: deliberately *not* React state or AuthContext, because the only consumer is the idle timer's interval tick, which needs the value **at tick time, not reactively** — routing it through context would re-render every auth consumer twice per purchase for no benefit, and would mean touching AuthContext's session logic for something unrelated to it. Same module-level pattern as `loadRazorpayCheckout.ts`'s memoized `scriptPromise`. Lives in `lib/` rather than `features/profile/` so `features/auth` doesn't have to import from `features/profile` to read it.
- **New** `src/features/auth/useIdleTimeout.ts` — `useIdleTimeout(enabled: boolean)`. Measures idle time against a `lastActivity` timestamp checked by a 15s interval, rather than re-arming `setTimeout`s, which is what makes the checkout suspension a single arithmetic adjustment instead of a teardown/rebuild of pending timers. Thresholds `WARNING_AFTER_MS = 15min`, `SIGN_OUT_AFTER_MS = 20min`; both toast strings derive their minute counts from the constants so the copy can't drift from the policy.
- Activity throttle: the `lastActivity` timestamp *is* the throttle — `if (Date.now() - lastActivityRef.current < 5000) return`, so a mousemove storm costs one comparison per event and nothing else, with no second timestamp to keep in sync.
- Listeners bound with `{ capture: true, passive: true }`. Capture phase is load-bearing, not incidental: `scroll` does not bubble, so a bubble-phase window listener would miss scrolling inside any nested scroll container.
- **Checkout suspension is a freeze, not an early return.** On a tick with `isCheckoutOpen()` true, `lastActivityRef.current += sinceLastTick` before returning, which holds the *measured* idle duration constant for as long as the overlay is open. A bare `return` would have let wall-clock time keep accumulating and fired the sign-out the instant a long checkout finished — the exact failure the guard exists to prevent. Resetting instead would be wrong in the other direction: it would hand a fresh full window to someone who genuinely walked away mid-checkout.
- Warning toast: `toast.warning("Still there?", { id, description, duration: Infinity, action: { label: "Stay signed in", onClick: resetIdleClock } })`. Fires once per idle cycle (`warnedRef`), dismissed by id on reset, on sign-out, and on unmount. Raw activity also resets the clock and dismisses the warning — after a genuine 15-minute absence, a returning user's first interaction keeping them signed in is the correct outcome, and the action button remains the explicit path for keyboard/touch users and for anyone who wants certainty.
- Sign-out: guarded by `signingOutRef` so it fires exactly once, dismisses the warning toast, calls the existing `useAuth().signOut()` (no duplicated logic), then shows `toast.info("Signed out after 20 minutes of inactivity.")` — the `Toaster` lives outside `ProtectedRoute` in `App.tsx`, so it survives the sign-out and the message is actually seen. Sign-out is the only action this timer can ever take — never a payment, cancellation, renewal, or downgrade (BR-001).
- Teardown removes every listener (matching capture flag), clears the interval, and dismisses the warning toast — no leaks.
- `src/features/auth/ProtectedRoute.tsx` — one call, `useIdleTimeout(Boolean(session))`. This component is the app's single layout route around `AppLayout`, so it stays mounted across every authenticated navigation: the timer arms once on sign-in and tears down on sign-out rather than restarting on each route change. Public `/auth/*` routes render outside it and are never covered.
- `src/features/profile/UpgradeButton.tsx` — `setCheckoutOpen(true)` immediately before `checkout.open()`; cleared in all three exits: `modal.ondismiss`, `verifyPayment()`'s new `finally` block (so it clears even if verification throws — a stuck-true flag would silently disable the idle timeout for the rest of the session), and the outer `catch` (covers `checkout.open()` itself throwing after the flag was set, where no overlay ever appeared). `verifyPayment` was restructured into `try`/`finally` for this; its existing success/failure toasts and `refreshProfile()` call are unchanged in behavior.
- `AuthContext.tsx` untouched — DEC-084's `onAuthStateChange`/`applySession` logic is byte-identical. No cross-tab syncing built, per scope.

**Verification:**
- `npx tsc -b` — clean.
- `npx eslint src/` — clean, back to the same 4-error pre-existing baseline (`badge.tsx`, `button.tsx`, `tabs.tsx`, `AuthContext.tsx`, all `react-refresh/only-export-components`). Worth recording that lint caught two *real* violations of mine on the first run: `react-hooks/purity` on `useRef(Date.now())` — reading the clock in a ref initializer runs during render and is impure. Fixed by seeding both timestamp refs to `0` and letting the effect set them, which is safe because nothing outside the effect ever reads them.
- `npm run build` — clean, built in 1.11s (the pre-existing >500 kB chunk-size advisory is unchanged and unrelated).
- `npm run dev` — server starts and serves clean on :5173, no compile or runtime errors in the Vite output.
- **Not verifiable from this sandbox (no browser automation available here):** the four interactive checks are genuinely *not* done and are explicitly **not** being claimed — (a) warning toast appears at 15 min, (b) "Stay signed in" resets the clock and dismisses it, (c) sign-out actually fires at 20 min of true inactivity, (d) **the one that actually matters** — starting a Razorpay checkout and leaving it open past 20 minutes does not sign the user out mid-flow. These need a human at the browser. Accelerated recipe handed to the user so the state machine can be exercised in ~1 minute instead of 35: temporarily set `WARNING_AFTER_MS = 20_000`, `SIGN_OUT_AFTER_MS = 35_000`, `TICK_INTERVAL_MS = 1_000`, `ACTIVITY_THROTTLE_MS = 500` in `useIdleTimeout.ts`, run all four checks, then revert the constants (nothing else in the file changes). This feature is **not** to be considered closed until check (d) in particular has been run live.

**Commit:** `d1d6d48` — "Add idle-session timeout with checkout suspension"
...
**Prompt:**
Combined Phase 9+10 build: (1) Profile page with a plan comparison (pulled from `premium_plans`) and an Upgrade button triggering Razorpay Test Mode checkout, premium status reflecting immediately without a manual refresh; (2) `razorpay-create-order`/`razorpay-verify-payment` Edge Functions built exactly to doc 11 §5.5/§5.6's existing spec; (3) Decision Workspace AI Insight batch gating — free tier capped to 1 insight (highest-urgency only, no partial teaser), premium keeps the existing up-to-3 batch; (4) Insights page built out for real — spend summaries, duplicate/overlap detection, lower-cost-alternative suggestions, AI insight summary, gated premium-exclusive with a locked/upsell state for free tier, not partial content; (5) a minimal tier indicator on Profile, plus a justified call on whether Decision Workspace also needs an inline note explaining the cap. Explicitly plan-first (no code) per `superpowers`' brainstorm-before-implementation rule for new features — three rounds of user review caught real bugs in the initial plan before any code was written (see below). Don't touch the IIT Capstone docs folder — flag doc impact instead.

**Skills invoked:**
- `superpowers` — full plan-mode brainstorm cycle before any implementation: three Explore agents researched the codebase in parallel, a Plan agent drafted the initial design, then three rounds of user-driven plan revision (receipt-length bug, signature-scrub-trigger misstatement, `PAY_002` code overload, missing anomaly audit log; then a critical premium-expiry replay bug and a duplicated-entitlement-check bug; then an unsafe race-fallback branch in `razorpay-verify-payment`) before the plan was approved and built. This is exactly the kind of bug systematic pre-implementation review is meant to catch — the replay-extension bug in particular (resending a valid verify-payment payload could have pushed `premium_expires_at` out indefinitely) would have been a real, exploitable free-premium bug if shipped as first drafted.
- `impeccable` was not separately invoked as a distinct pass — new UI (Profile plan comparison, tier badge, Insights upsell/summary cards) reuses this app's existing Product-register card/button/badge patterns (`rounded-lg border border-border bg-card p-6`, existing `Badge`/`Button` variants) rather than introducing new visual treatment, so no new design surface needed a dedicated audit.

**What was done:**
- `supabase/functions/_shared/http.ts` — exported the previously-private `safeCompare` for reuse in signature verification.
- New `supabase/functions/_shared/razorpay-client.ts` — `createRazorpayOrder()` (plain `fetch` + HTTP Basic Auth to Razorpay's Orders API, no SDK — Razorpay's npm package targets Node/axios, no Deno build) and `verifyRazorpaySignature()` (Web Crypto HMAC-SHA256, `safeCompare` constant-time check).
- New `supabase/functions/razorpay-create-order/index.ts` — looks up the requested `premium_plans` row (never trusts a client-supplied amount/currency), rejects the free plan, builds a Razorpay-compliant receipt (`sub_<timestamp>_<last8ofUserId>`, kept under Razorpay's 40-char cap — the initial full-UUID version would have been ~54 chars), creates the order, inserts a `payment_transactions` row before responding. Introduced `PAY_004` ("plan not found") as a code distinct from `PAY_002` ("order not found" in verify-payment) rather than overloading one code with two meanings, and `PAY_005` for upstream Razorpay/DB failures — both need recording in doc 11's error-code table (not done here, flagged below).
- New `supabase/functions/razorpay-verify-payment/index.ts` — order lookup (generic `PAY_002` for missing-or-not-owned, no existence leak); idempotency check keyed on `razorpay_payment_id` (never `razorpay_signature`, which the existing `payment_transactions_scrub_signature` trigger unconditionally nulls on any insert/update touching it — worked with, not around); signature verification with a `PAY_001` + failed-status write on mismatch; a CAS `UPDATE ... WHERE status = 'created'` for the actual grant, with a race-fallback branch that re-selects and branches on the real committed state (verified+same-payment_id → grant, `failed` → `PAY_001`, verified+different-payment_id → `PAY_002` + a new `payment_verification_anomaly` audit log entry) rather than blindly granting on any 0-row CAS result. `premium_expires_at` is computed from the transaction row's own stored `verified_at` plus the plan's `duration_days` — never wall-clock `now()` — so every replay (genuine duplicate callback, or the benign-race fallback) converges on the exact same expiry instead of extending it further on each replay.
- `src/features/auth/AuthContext.tsx` — widened `UserProfile` with typed `is_premium`/`premium_expires_at`/`premium_source`; added `refreshProfile()` (a narrow, additive re-fetch, confirmed no such function existed before) so a successful purchase updates the tier without a full session reload.
- New `src/features/premium/premium-utils.ts` — `isPremiumActive(profile)`, frontend-only UI convenience check. **No Deno-side reimplementation**: confirmed `public.user_has_active_premium(uuid)` already exists live in the schema (`17_SubSense_Migration_v2.sql:459-473`, `security definer`, granted to `authenticated`) and already implements this exact rule — every Edge Function that needs a server-side premium check calls it via `supabaseAdmin.rpc("user_has_active_premium", {...})` instead of duplicating the boolean logic.
- Extracted `computeTotalsByCurrency`/`CurrencyTotals` out of `DecisionWorkspacePage.tsx` into `src/features/subscriptions/subscription-utils.ts` as named exports (pure move, confirmed exact prior signature via grep first) — reused by both Decision Workspace (unchanged, Path A) and the new Insights Edge Function (its own Deno-side port, same cross-runtime-boundary duplication pattern as `TOP_URGENT_LIMIT`/`WORKSPACE_BATCH_LIMIT`).
- `src/features/profile/` — `ProfilePage.tsx` rewritten (tier badge + `PlanComparisonCard`); `PlanComparisonCard.tsx` renders generically from whatever active `premium_plans` rows exist, not hardcoded to today's 2 seeded rows; `UpgradeButton.tsx` orchestrates create-order → Razorpay checkout widget → verify-payment → `refreshProfile()` + toast, every step behind the explicit click (BR-001); `loadRazorpayCheckout.ts` lazily injects `checkout.js` only on Upgrade click, not a static `index.html` tag; `razorpay-types.d.ts` minimal ambient `Window.Razorpay` typing, feature-local; `TierBadge.tsx`.
- `src/features/ai-insights/ai-insight-utils.ts` — added `FREE_TIER_WORKSPACE_LIMIT = 1` (DEC-082) alongside `TOP_URGENT_LIMIT`. `useWorkspaceAiInsights.ts` now takes an `isPremium` param and caps `candidates` accordingly while still exposing the uncapped `allCandidateCount`. `DecisionWorkspacePage.tsx` passes `isPremiumActive(profile)` in and renders a muted inline note ("Free plan shows your single most urgent insight. Upgrade to see up to 3.") only when the cap actually clipped something — never to premium users or to a free user whose full urgent set was already ≤1. Chose to add this note rather than stay silent: DEC-082 forbids partial-teaser *cards*, not explanatory text, and without it a free user with genuinely more urgent renewals can't tell "nothing else is urgent" from "something's hidden."
- `supabase/functions/ai-generate-insight/index.ts` — closed the server-side gating gap: the `"workspace"` scope previously always generated up to `WORKSPACE_BATCH_LIMIT = 3` regardless of caller, meaning a free-tier client could trigger (and get billed/cached for) 3x the OpenAI generations it ever displayed, with nothing stopping a modified client from requesting more. Now looks up `user_has_active_premium` via RPC and slices to `FREE_TIER_WORKSPACE_LIMIT` when not premium.
- New `supabase/functions/_shared/insight-summary-prompt.ts` and `supabase/functions/insights-generate-summary/index.ts` — hard-gated (`403 INS_001`) via the same `user_has_active_premium` RPC before touching any subscription data. Computes spend summary (Deno port of `computeTotalsByCurrency`), duplicate/overlap detection (exact same-`catalog_id` groups and category-overlap groups, both grouped by currency to avoid cross-currency summing), and lower-cost-alternative suggestions as a **deterministic intra-portfolio category-average comparison** — no competitor-pricing data exists anywhere in this schema, so inventing one would violate the same no-fabricated-financial-claims discipline `ai-generate-insight`'s `financial_impact` already enforces. Exactly one OpenAI call frames the deterministic facts into prose (never invents its own numbers, per the system prompt).
- `src/features/insights/` — `InsightsPage.tsx` checks `isPremiumActive(profile)` client-side first: free users render only `InsightsUpsellCard.tsx` and **no Edge Function call is made at all** (not just hidden — never requested); premium users call `insights-generate-summary` and render spend/duplicate/lower-cost/AI-summary sections.
- Flagged, not edited: doc 04 (Insights/Profile blueprints), doc 08 (new DEC recording DEC-082's gating now implemented plus the CAS/self-healing and inline-note decisions), doc 10 (one text fix — `user_has_active_premium` reader is no longer "planned, not yet implemented"), doc 11 (mark §5.5/§5.6 built, add a new §5.x for `insights-generate-summary`, record the new `PAY_004`/`PAY_005` codes and the free-tier 1-vs-3 workspace behavior), doc 16 (Phase 9/10 → complete), and doc 06 (no entries yet for the new UI surfaces — flagged as a judgment call, not done here).

**Verification:**
- `npx tsc -b`, `npx eslint src/`, `npm run build` — all clean, same pre-existing 4-error ESLint baseline (`badge.tsx`/`button.tsx`/`tabs.tsx`/`AuthContext.tsx` fast-refresh warnings). Production build succeeds (only the pre-existing >500kB chunk-size advisory, unrelated).
- All five new/changed Edge Function files are Deno runtime, confirmed (again) out of scope for both `tsc -b` and `eslint src/` — read back `razorpay-verify-payment/index.ts` end-to-end after writing it to re-confirm the CAS race-fallback branches exactly mirror the idempotency-check branches (no silent fall-through to granting), matching the final approved plan.
- **Not verifiable from this sandbox**: no live Supabase or Razorpay Test Mode access. The user needs to: confirm the exact `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` secret names in Supabase Dashboard → Edge Functions → Secrets match what this code reads (assumed per a prior `BUILD_LOG.md` note that they're already provisioned); deploy; walk through a real Test Mode purchase on `/profile` and confirm the tier badge and Decision Workspace cap update without a manual refresh; confirm free-tier `/insights` never fires the Edge Function call (Network tab) while premium does; resubmit a duplicate `razorpay-verify-payment` payload and confirm idempotent success rather than a double-credit or error.

**Commit:** `ddefaa7` — "Add Phase 9+10: Insights, premium gating, Razorpay Test Mode demo"
...
**Prompt:**
Plan Phase 8 (Shared Subscriptions) per doc 16, planning-only first per the same workflow as Phases 6/7 (stop for approval before writing code). Architecture locked under DEC-080 (doc 08 v1.53): a DB trigger generates `payment_requests` on `subscriptions.next_renewal_date` change (not lazy-generate, not a cron job), "Manage sharing" on Subscription Details is the single entry point for setup and ongoing management, equal-split rebalances automatically, member removal never cascade-cancels open requests. After the plan surfaced two real gaps DEC-080's text didn't resolve, the user answered both via `AskUserQuestion`, then reviewed the written plan and required three more additions before approving: (1) documenting the RLS-unselectability tradeoff the nullable-`user_id` fix creates, in both the plan and a migration comment; (2) documenting the equal-split rounding-remainder tradeoff the same way; (3) resolving a "Mark Paid" naming collision between the already-shipped `MarkPaidDialog` (DEC-064, "I paid the provider") and the new C-021 owner action (DEC-080, "this member paid me back") via distinct copy, not just distinct components. Once revised, approved with "same bite-sized, one-step-at-a-time style as Phase 6/7" build cadence.

Investigation before planning found the schema for `shared_subscriptions`/`shared_members`/`payment_requests` — including RLS, the archive-cascade trigger, currency-validation triggers, and the `payment_requests` status-transition-authorization trigger (DEC-037) — already exists and is already live, shipped years earlier in `17_SubSense_Migration_v2.sql`; this was not a greenfield schema build. That same investigation surfaced a real, unaddressed gap: DEC-080's generation trigger only fires on `next_renewal_date` change, but sharing setup (or a new member joining) never changes that column, so a newly-shared subscription would generate zero `payment_requests` until the next renewal cycle — resolved via `AskUserQuestion` ("generate immediately on member join too," confirmed). A second gap: `reminders.user_id`/`notifications.user_id` are `NOT NULL`, but `shared_members.user_id` is nullable (a member may have no SubSense account) — neither table could hold a row for an unlinked member's reminder. The first proposed resolution ("skip reminders for unlinked members entirely") was rejected by the user as treating a designed-for case as an edge case; the user's own counter-proposal (loosen both columns to nullable, scoped CHECK constraints, branch recipient-email resolution in `send-reminder-email`) was adopted after verifying it was sufficient — a full read of `send-reminder-email/index.ts` confirmed nothing else in the send path depends on `user_id` for the `shared_payment` case (no name interpolation, no timezone/preference lookup), so the blast radius was exactly the two schema constraints plus one recipient-resolution branch.

**What was done:**

Database (external docs folder, `31_SubSense_Shared_Payment_Requests_v1.0.sql` — not checked into this repo, matching every prior migration file's location; run by the user directly against Supabase, confirmed clean):
- Loosened `reminders.user_id`/`notifications.user_id` to nullable, each backed by a scoped replacement CHECK (`user_id IS NOT NULL OR payment_request_id IS NOT NULL`), with an inline comment recording the resulting RLS-unselectability tradeoff as deliberate.
- `generate_payment_request_for_member()` / `generate_payment_requests_for_shared_subscription()` — the generation helper (mirrors `29_SubSense_Reminder_Lifecycle_Patch_v1.0.sql`'s callable-helper pattern) and its fan-out, snapshotting `amount`/`currency`, auto-creating the matching `shared_payment` reminder only when a request row was actually inserted (not a no-op conflict).
- `subscriptions_generate_payment_requests` trigger — `AFTER UPDATE OF next_renewal_date`, same event DEC-070's reminder triggers already react to.
- `rebalance_equal_split_members()` — equal-split recompute, with the rounding-remainder tradeoff noted inline at the division.
- `shared_members_generate_initial_request` (fires on member insert, closes the initial-generation gap) and `shared_members_rebalance_on_remove` (rebalance only, no cascade-cancel) triggers.
- All 5 new functions `SECURITY DEFINER`, idempotent (`CREATE OR REPLACE`, `DROP ... IF EXISTS`), single transaction (no two-step split needed — that convention is only for `ALTER TYPE ... ADD VALUE`).

Backend:
- `supabase/functions/send-shared-payment-reminder/index.ts` (new) — doc 11 §5.3. Owner-JWT auth (`requireAuthenticatedUser`, not service-role). Verifies caller owns the parent `shared_subscriptions` row; finds-or-creates the underlying `reminders` row; delegates the actual send to `send-reminder-email`'s single mode via an internal HTTP call authenticated with the service-role key (read server-side only) — reuses all of its retry/render/delivery-tracking logic with zero duplication, rather than refactoring that working Phase 6 code.
- `supabase/functions/send-reminder-email/index.ts` (modified) — `ReminderRow.user_id` now `string | null`; `userIds` batch-fetch filters nulls; `payment_requests` select extended to join `shared_members(email, display_name)`; new `resolveRecipientEmail()` branches `user_id` set → `users.email`, `user_id` null → the joined `shared_members.email`.

Frontend (`src/features/shared-subscriptions/`, `src/components/shared-subscriptions/`):
- `shared-subscription-utils.ts` — row interfaces/SELECT constants matching the live schema, label maps, `getMemberDisplayName()`, and `canOwnerMarkPaid`/`canMemberReportPaid`/`canSendReminder` predicates mirroring `payment_requests_validate_transition`'s allowed transitions (DEC-037) — UI-affordance logic only, not a re-implementation of the DB-enforced boundary.
- `useSharedSubscription(subscriptionId, currency)` — one subscription's sharing state for "Manage sharing": `createShare`, `addMember`, `editMember`, `removeMember` (soft), `ownerMarkPaid`, `reportPaid` (direct Path A `payment_requests` updates — no Edge Function, already fully authorized by the existing DB trigger + RLS), `sendReminder` (invokes the new Edge Function).
- `useSharedSubscriptionsList()` — batch-fetched landing-page data (every `shared_subscriptions` row the caller can see, RLS-scoped automatically) plus the same three payment-action mutations, needed here specifically because a linked member has no RLS access to `SubscriptionDetailsPage` (owner-scoped) — this landing page is their only surface to ever report "I've Paid."
- `ConfirmDialog.tsx` — first generic implementation of C-017 (doc 06 v1.24 had named "Reminder confirmation"/"Payment status confirmation" as variants with no concrete component behind them yet).
- `SharedMemberRow.tsx` (C-019), `AddEditMemberForm.tsx` (C-020), `PaymentRequestItem.tsx` (C-021).
- `MarkPaidDialog.tsx`/`SubscriptionCard.tsx` — naming-collision fix: `MarkPaidDialog` now takes a `name` prop, used in its dialog title ("Mark {name} as paid") and success toast, so it's never mistaken for C-021's new "Mark [member]'s payment received" action. The card's own "Paid" button stays unchanged (the subscription name is already directly above it).
- `SubscriptionDetailsPage.tsx` — replaced the placeholder "Shared Members" section with the full `useSharedSubscription`-backed UI: split-method-picker setup dialog, member roster (owner-only Add/Edit/Remove), payment-request history (resolving names via the joined `shared_members` even for since-removed members).
- `SharedSubscriptionsPage.tsx` — full rebuild on `useSharedSubscriptionsList`, page-shell pattern matching `SubscriptionDetailsPage`; member-roster editing intentionally stays owner-only via "Manage" → Subscription Details (DEC-080: single entry point, not a duplicated flow), but payment actions (Mark received / I've Paid / Send Reminder) are fully interactive here since this is a linked member's only reachable surface.

**Verification:**
- `npx tsc -b`, `npx eslint src/` (repo-wide), `npm run build` — all clean after every individual piece and again at the end; same pre-existing 4-error baseline (`badge.tsx`/`button.tsx`/`tabs.tsx`/`AuthContext.tsx` fast-refresh warnings, unrelated to this work).
- DEC-031/037 write-boundary review (the Phase 8 analog of Phase 7's BR-001 grep): confirmed zero `.from("subscriptions")` writes anywhere in `src/features/shared-subscriptions/`; every write across both hooks is exactly one of the expected `shared_subscriptions`/`shared_members`/`payment_requests` mutations; confirmed zero direct Supabase calls in any presentational component under `src/components/shared-subscriptions/` (everything routes through the two hooks).
- `send-shared-payment-reminder/index.ts` grepped: exactly one `.insert()` (into `reminders`), zero `.update()/.delete()/.upsert()`, zero `.from("subscriptions")`/`.from("shared_subscriptions")`.
- SQL migration: run by the user directly against Supabase, confirmed clean (no errors; both new CHECK constraints and all new trigger names verified present via `pg_constraint`/`pg_trigger`).
- A real `react-hooks/set-state-in-effect` lint snag (same rule that hit `useAiInsight.ts` in Phase 7) had a narrower actual trigger than assumed: it only tolerates a `load()` function *declared inside* the `useEffect` body, not a call to an externally-declared one, regardless of whether that function's own `setState` calls happen after an `await`. Fixed in both `useSharedSubscription.ts` and `useSharedSubscriptionsList.ts` by nesting `load()` inside the effect and using a `reloadTrigger` counter bump (in the effect's dependency array) for post-mutation refetches, rather than calling `load()` directly from mutation handlers.
- **Not verified from this sandbox**: the actual Edge Function calls, the real HTTP round-trip between `send-shared-payment-reminder` and `send-reminder-email`, real Resend delivery to an unlinked member's email, or any live UI walkthrough — no browser, no deployed functions checked from here yet. Flagged rather than claimed, same standing gap as every previous phase.

**Commit:** `258c4b7` — "Implement Phase 8: Shared Subscriptions (DEC-080)" — 13 files changed, 1397 insertions(+), 21 deletions(-)
...
**Commit logged:** `b076918` — "Add BUILD_LOG.md entry for Cost Comparison Card rename" (2026-08-06 21:12) — 1 file changed, 25 insertions(+)
